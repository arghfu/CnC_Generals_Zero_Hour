	...
	for( i = 0; i < numPolys; i++ )
	{
		...
		for( j = 0; j < MAX_POLYGON_NEIGHBORS; j++ )
		{
			...
			if( polyNeighbor->neighbor[ j ].neighborIndex != NO_NEIGHBOR )
			{
				...
				//
				if( BitTest_( otherNeighbor->status, POLY_PROCESSED ) )
					continue;  // for j
			...
			//
			if( BitTest_( polyNeighbor->status, POLY_VISIBLE ) )
			{
				...
				else if( BitTest_( otherNeighbor->status, POLY_VISIBLE ) == FALSE )
				{
					...
					addSilhouetteEdge(meshIndex, polyNeighbor, otherNeighbor );
							 ...
							 BitTest_( otherNeighbor->status, POLY_VISIBLE ) )
			{
