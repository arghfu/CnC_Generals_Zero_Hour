...
static void drawButtonText( GameWindow *window, WinInstanceData *instData )
{
	...
	text->setWordWrapCentered( BitTest_( instData->getStatus(), WIN_STATUS_WRAP_CENTERED ));
	text->setWordWrap(size.x);
	// get the right text color
	if( BitTest_( window->winGetStatus(), WIN_STATUS_ENABLED ) == FALSE )
	{
		textColor = window->winGetDisabledTextColor();
		dropColor = window->winGetDisabledTextBorderColor();
	}  // end if, disabled
	else if( BitTest_( instData->getState(), WIN_STATE_HILITED ) )
	{
	...
	// where to draw
	if( BitTest_( window->winGetStatus(), WIN_STATUS_SHORTCUT_BUTTON ) )
	{
...
void W3DGadgetPushButtonDraw( GameWindow *window, WinInstanceData *instData )
{
	...
	if( BitTest_( window->winGetStatus(), WIN_STATUS_ENABLED ) == FALSE )
	{
		...
		if( BitTest_( instData->getState(), WIN_STATE_SELECTED ) )
		{
																	 ...
																	 WinInstanceData *instData )
{
	...
	{
		if( BitTest_( instData->getState(), WIN_STATUS_USE_OVERLAY_STATES ) )
		{
			ICoord2D size, start;
			// get window position
																	 ...
																	 WinInstanceData *instData )
{
	...
	if( !BitTest_( window->winGetStatus(), WIN_STATUS_USE_OVERLAY_STATES ) )
	{
	...
	if( image )
	{
		...
		{	
			...
			if( !BitTest_( window->winGetStatus(), WIN_STATUS_ENABLED ) )
			{
				if( !BitTest_( window->winGetStatus(), WIN_STATUS_NOT_READY ) )
				{
					...
					if( !BitTest_( window->winGetStatus(), WIN_STATUS_ALWAYS_COLOR ) )
					{
	...
	if( BitTest_( window->winGetStatus(), WIN_STATUS_FLASHING ) )
	{
	...
	if( BitTest_( window->winGetStatus(), WIN_STATUS_USE_OVERLAY_STATES ) )
	{
		...
		if( pushedOverlayIcon && hilitedOverlayIcon )
		{
			if(BitTest_(window->winGetStatus(), WIN_STATUS_ENABLED))
			{
				if (BitTest_( instData->getState(), WIN_STATE_HILITED ))
				{
					if( BitTest_( instData->getState(), WIN_STATE_SELECTED ) )
					{
  			...
  			else if( BitTest_( instData->getState(), WIN_STATE_SELECTED ) )
 				{
