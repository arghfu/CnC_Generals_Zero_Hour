...
public:
	...
	*/
	...
	Bool getIsUndetectedDefector(void) const { return BitTest_(m_privateStatus, UNDETECTED_DEFECTOR); }
	void friend_setUndetectedDefector(Bool status);

	inline Bool isOffMap() const { return BitTest_(m_privateStatus, OFF_MAP); }

	inline Bool isCaptured() const { return BitTest_(m_privateStatus, CAPTURED); }
  ...
  StealthUpdate*          getStealth() const { return m_stealth; }
	...
	inline Bool testScriptStatusBit(ObjectScriptStatusBit b) const { return BitTest_(m_scriptStatus, b); }
