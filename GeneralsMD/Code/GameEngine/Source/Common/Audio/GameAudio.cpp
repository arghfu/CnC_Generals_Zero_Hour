...
Bool AudioManager::shouldPlayLocally(const AudioEventRTS *audioEvent)
{
	...
	if (!BitTest_(ei->m_type, (ST_PLAYER | ST_ALLIES | ST_ENEMIES | ST_EVERYONE))) {
		DEBUG_CRASH(("No player restrictions specified for '%s'. Using Everyone.\n", ei->m_audioName.str()));
		return TRUE;
	}

	if (BitTest_(ei->m_type, ST_EVERYONE)) {
		return TRUE;
	}

	Player *owningPlayer = ThePlayerList->getNthPlayer(audioEvent->getPlayerIndex());

	if (BitTest_(ei->m_type, ST_PLAYER) && BitTest_(ei->m_type, ST_UI) && owningPlayer == NULL) {
	...
	if (BitTest_(ei->m_type, ST_PLAYER))  {
		return owningPlayer == localPlayer;
	}

	if (BitTest_(ei->m_type, ST_ALLIES)) {
	...
	if (BitTest_(ei->m_type, ST_ENEMIES)) {
