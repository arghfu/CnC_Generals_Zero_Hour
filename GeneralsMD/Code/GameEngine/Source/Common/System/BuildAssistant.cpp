																											 ...
																											 Player *thePlayer)
{
	...
	for( them = iter->first(); them; them = iter->next() )
	{
		...
		if( BitTest_( options, IGNORE_STEALTHED ) )
		{
			if( rel != ALLIES )
			{
				...
				{
					if( BitTest_( options, FAIL_STEALTHED_WITHOUT_FEEDBACK ) )
					{
																											 ...
																											 Player *player)
{
	...
	if( BitTest_( options, SHROUD_REVEALED ) )
	{
		{
			Int x, y;
	...
	//
	if( BitTest_( options, NO_OBJECT_OVERLAP ) )
	{
	...
	//
	if( BitTest_( options, NO_ENEMY_OBJECT_OVERLAP ) )
	{
		LegalBuildCode code = isLocationClearOfObjects(worldPos, build, angle, builderObject, options, player);
		if( code != LBC_OK )
		{
	...
	if( BitTest_( options, CLEAR_PATH ) && builderObject && !builderObject->isKindOf( KINDOF_IMMOBILE ) )
	{
	...
	if( BitTest_( options, TERRAIN_RESTRICTIONS ) )
	{
