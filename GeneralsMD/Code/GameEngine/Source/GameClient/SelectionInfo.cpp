...
UnsignedInt getPickTypesForContext( Bool forceAttackMode )
{
	...
	if (command != NULL) {
		if (BitTest_( command->getOptions(), ALLOW_MINE_TARGET)) {
			types |= PICK_TYPE_MINES;
		}

		if (BitTest_( command->getOptions(), ALLOW_SHRUBBERY_TARGET ) ) {
...
//		}
		...
		// For efficiency.
		if (BitTest_(retVal, PICK_TYPE_MINES | PICK_TYPE_SHRUBBERY)) {
			break;
		}
	}
...
{
	if (BitTest_(pickTypes, PICK_TYPE_SELECTABLE)) {
		outMask.set(KINDOF_SELECTABLE);
	}

	if (BitTest_(pickTypes, PICK_TYPE_SHRUBBERY)) {
		outMask.set(KINDOF_SHRUBBERY);
	}

	if (BitTest_(pickTypes, PICK_TYPE_MINES)) {
		outMask.set(KINDOF_MINE);
	}

	if (BitTest_(pickTypes, PICK_TYPE_FORCEATTACKABLE)) {
