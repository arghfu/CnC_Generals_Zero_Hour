...
void InGameUI::createMouseoverHint( const GameMessage *msg )
{
	...
	while (window)
	{
		...
		if (!BitTest_( window->winGetStatus(), WIN_STATUS_SEE_THRU ))
		{
			underWindow = true;
			break;
		}
		...
		case MOUSEMODE_GUI_COMMAND:
			{
				...
				if( m_pendingGUICommand )
				{
					...
					else if( BitTest_( m_pendingGUICommand->getOptions(), COMMAND_OPTION_NEED_TARGET ) )
					{
...
void InGameUI::setGUICommand( const CommandButton *command )
{
	...
	if( command )
	{
		...
		if( BitTest_( command->getOptions(), COMMAND_OPTION_NEED_TARGET ) == FALSE )
		{
	...
	// set the mouse cursor for commands that need a targeting or to normal with no command
	if( command && BitTest_( command->getOptions(), COMMAND_OPTION_NEED_TARGET ) && !command->isContextCommand() )
	{
...
Bool InGameUI::canSelectedObjectsDoSpecialPower( const CommandButton *command, const Object *objectToInteractWith, const Coord3D *position, SelectionRules rule, UnsignedInt commandOptions, Object* ignoreSelObj ) const
{
	...
	//3) NEED_TARGET_POS
	Bool doAtPosition = BitTest_( command->getOptions(), NEED_TARGET_POS );
	Bool doAtObject = BitTest_( command->getOptions(), COMMAND_OPTION_NEED_OBJECT_TARGET );
			 ...
			 it != m_worldAnimationList.end(); /*empty*/ )
	{
		...
		if( wad && TheGameLogic->isGamePaused() == FALSE )
		{
			...
			if( TheGameLogic->getFrame() >= wad->m_expireFrame ||
					(BitTest_( wad->m_options, WORLD_ANIM_PLAY_ONCE_AND_DESTROY ) &&
					 BitTest_( wad->m_anim->getStatus(), ANIM_2D_STATUS_COMPLETE )) )
			{
		...
		if( BitTest_( wad->m_options, WORLD_ANIM_FADE_ON_EXPIRE ) )
		{
