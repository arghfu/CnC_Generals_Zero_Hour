...
static Bool buttonTriggersOnMouseDown(GameWindow *window)
{
	...
	Bool onDown = BitTest_( window->winGetStatus(), WIN_STATUS_ON_MOUSE_DOWN);

	// Checkboxes always trigger on mouse down. jba [8/6/2003]
	if (BitTest_( window->winGetStatus(), WIN_STATUS_CHECK_LIKE )) {
		onDown = true;
	}
	return onDown;
}
																						...
																						WindowMsgData mData2 )
{
	...
	switch( msg ) 
	{
		...
		case GWM_MOUSE_ENTERING:
		{
			...
			if(window->winGetParent() && BitTest_(window->winGetParent()->winGetStyle(),GWS_HORZ_SLIDER) )
			{
		...
		case GWM_MOUSE_LEAVING:
		{
			...
			//
			if( BitTest_( window->winGetStatus(), WIN_STATUS_CHECK_LIKE ) == FALSE )
		...
		case GWM_LEFT_DOWN:
		{
			...
			if( BitTest_( window->winGetStatus(), WIN_STATUS_CHECK_LIKE ) )
			{
				...
				if( BitTest_( instData->m_state, WIN_STATE_SELECTED ) )
					BitClear( instData->m_state, WIN_STATE_SELECTED );
				else
		...
		case GWM_LEFT_UP:
		{
			...
			//
			if( BitTest_( instData->getState(), WIN_STATE_SELECTED ) &&
					BitTest_( window->winGetStatus(), WIN_STATUS_CHECK_LIKE ) == FALSE )
			{
		...
		case GWM_RIGHT_DOWN:
		{
			...
			if( BitTest_( instData->getStatus(), WIN_STATUS_RIGHT_CLICK ) )
			{
		...
		case GWM_CHAR:
		{
			switch( mData1 )
			{
				...
				case KEY_SPACE:
				{
					...
					if( BitTest_( mData2, KEY_STATE_UP ) )
					{
				...
				case KEY_TAB:
				{
					...
					if( BitTest_( mData2, KEY_STATE_DOWN ) )
...
void GadgetCheckLikeButtonSetVisualCheck( GameWindow *g, Bool checked )
{
	...
	if( BitTest_( g->winGetStatus(), WIN_STATUS_CHECK_LIKE ) == FALSE )
	{
...
Bool GadgetCheckLikeButtonIsChecked( GameWindow *g )
{
	...
	return BitTest_( instData->m_state, WIN_STATE_SELECTED );
