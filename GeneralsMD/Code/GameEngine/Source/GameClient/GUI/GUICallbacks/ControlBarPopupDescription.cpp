...
//			m_buildToolTipLayout = NULL;
	...
	if(!cmdButton)
		return;
	if(BitTest_(cmdButton->winGetStyle(), GWS_PUSH_BUTTON))
	{
	...
	else
	{
		...
		if(!BitTest_(cmdButton->winGetStyle(), GWS_USER_WINDOW) && !BitTest_(cmdButton->winGetStyle(), GWS_STATIC_TEXT))
			return;
...
void ControlBar::repopulateBuildTooltipLayout( void )
{
	if(!prevWindow || !m_buildToolTipLayout)
		return;
	if(!BitTest_(prevWindow->winGetStyle(), GWS_PUSH_BUTTON))
		return;
