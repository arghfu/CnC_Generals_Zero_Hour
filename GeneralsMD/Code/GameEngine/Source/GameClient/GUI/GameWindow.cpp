...
GameWindow *GameWindow::findPrevLeaf( void )
{
	...
	if( leaf->m_prev ) 
	{
		...
		while( leaf->m_child && 
					 BitTest_( leaf->m_status, WIN_STATUS_TAB_STOP ) == FALSE )
		{
...
GameWindow *GameWindow::findNextLeaf( void )
{
	...
	if( leaf->m_next ) 
	{
		...
		for( leaf = leaf->m_next; leaf; leaf = leaf->m_child )
			if( leaf->m_child == NULL || BitTest_( leaf->m_status,
																						WIN_STATUS_TAB_STOP ) )
				return leaf;
			...
			if( leaf->m_parent && leaf->m_next ) 
			{
				...
				for( leaf = leaf->m_next; leaf; leaf = leaf->m_child )
					if( leaf->m_child == NULL || 
							BitTest_( leaf->m_status, WIN_STATUS_TAB_STOP ) )
						return leaf;
...
{
  return BitTest_( m_status, WIN_STATUS_ENABLED );
...
Int GameWindow::winHide( Bool hide )
{
	...
	if( hide )
	{
		...
		//
		if( !BitTest_( m_status, WIN_STATUS_NO_FLUSH ) )
			freeImages();
...
Bool GameWindow::winIsHidden( void )
{
	...
	return BitTest_( m_status, WIN_STATUS_HIDDEN );
...
void GameWindow::winSetFont( GameFont *font )
{
	...
	if( BitTest_( m_instData.getStyle(), GWS_SCROLL_LISTBOX ) )
		GadgetListBoxSetFont( this, font );
	else if( BitTest_( m_instData.getStyle(), GWS_COMBO_BOX ) )
		GadgetComboBoxSetFont( this, font );
	else if( BitTest_( m_instData.getStyle(), GWS_ENTRY_FIELD ) )
		GadgetTextEntrySetFont( this, font );
	else if( BitTest_( m_instData.getStyle(), GWS_STATIC_TEXT ) )
...
void GameWindow::winSetEnabledTextColors( Color color, Color borderColor )
{
	...
	if( BitTest_( m_instData.getStyle(), GWS_COMBO_BOX ) )
...
Int GameWindow::winDrawWindow( void )
{
	...
	if( BitTest_( m_status, WIN_STATUS_HIDDEN ) == FALSE && m_draw )
...
GameWindow *GameWindow::winPointInChild( Int x, Int y, Bool ignoreEnableCheck, Bool playDisabledSound )
{
	...
	for( child = m_child; child; child = child->m_next ) 
	{
		...
		{
			Bool enabled = ignoreEnableCheck || BitTest_( child->m_status, WIN_STATUS_ENABLED );
			Bool hidden = BitTest_( child->m_status, WIN_STATUS_HIDDEN );
			if( !hidden )
			{
				if( enabled )
				{
...
GameWindow *GameWindow::winPointInAnyChild( Int x, Int y, Bool ignoreHidden, Bool ignoreEnableCheck )
{
	...
	for( child = m_child; child; child = child->m_next ) 
	{
				...
				y >= origin.y && y <= origin.y + child->m_size.y )
		{
			...
			if( !(ignoreHidden == TRUE &&	BitTest_( child->m_status, WIN_STATUS_HIDDEN )) )
