																							...
																							GadgetGameMessage gadgetMessage )
{
	...
	//rethink it.
	if( obj && BitTest_( commandButton->getOptions(), SINGLE_USE_COMMAND ) )
	{
	...
	if( BitTest_( commandButton->getOptions(), COMMAND_OPTION_NEED_TARGET ) )
	{
	...
	else switch( commandButton->getCommandType() )
	{
      ...
      //what if container is subdued... assert a logic failure, perhaps?
		...
		case GUI_COMMAND_EVACUATE:
		{
			...
			if (BitTest_(commandButton->getOptions(), NEED_TARGET_POS) == FALSE) {
