...
//	TheInGameUI->deselectAllDrawables();
	...
	for( i = 0; i < MAX_PURCHASE_SCIENCE_RANK_1; i++ )
	{
		...
		if( commandButton == NULL || BitTest_( commandButton->getOptions(), SCRIPT_ONLY ) )
		{
...
Bool CommandButton::isValidToUseOn(const Object *sourceObj, const Object *targetObj, const Coord3D *targetLocation, CommandSourceType commandSource) const
{
	...
	if( BitTest_( m_options, COMMAND_OPTION_NEED_OBJECT_TARGET ) && !targetObj )
	{
		return false;
	}
	...
	if( BitTest_( m_options, NEED_TARGET_POS ) && !targetLocation )
	{
		if( targetObj )
		{
...
{
	return BitTest_( m_options, CONTEXTMODE_COMMAND );
}
...
void ControlBar::setControlCommand( GameWindow *button, const CommandButton *commandButton )
{
	...
	//
	if( BitTest_( commandButton->getOptions(), CHECK_LIKE ))
...
void ControlBar::triggerRadarAttackGlow( void )
{
	...
	if(BitTest_(m_radarAttackGlowWindow->winGetStatus(),WIN_STATUS_ENABLED) == TRUE)
...
void ControlBar::updateRadarAttackGlow ( void )
{
	...
	{
		m_radarAttackGlowWindow->winEnable(!BitTest_(m_radarAttackGlowWindow->winGetStatus(),WIN_STATUS_ENABLED));
	}
...
void ControlBar::populateSpecialPowerShortcut( Player *player)
{
	...
	for( i = 0; i < m_currentlyUsedSpecialPowersButtons; i++ )
	{
		...
		else
		{
			...
			if( BitTest_( commandButton->getOptions(), NEED_UPGRADE ) )
			{
			...
			//
			if( BitTest_( commandButton->getOptions(), NEED_SPECIAL_POWER_SCIENCE ) )
			{
